...
| Peak detection | Resolution is mass-, shape-, abundance- and signal-to-noise-dependent; no fixed separation guarantees two resolvable ions. | Local maxima and physical-FWHM suppression are deterministic heuristics. Empirical or Gaussian non-negative fits expose rank, condition and residual diagnostics and withhold inseparable traces. Documentation no longer promises universal resolution. **Conservative heuristic.** |
...
| Detector echoes | Detector ringing is instrumental and must not be inferred merely from low m/z or formula absence. | Classification requires recurring calibrated time-bin delays behind several stronger parents plus primary-normalised level and change correlation. Supporting-only patterns remain visible. Formula and saved-review evidence veto suppression. **Conservative heuristic.** |
