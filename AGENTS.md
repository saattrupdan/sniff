...
- `src/sniff/reference/pubchem_enrichment.json` is a bounded, bundled offline snapshot,
  generated by `scripts/fetch_pubchem_enrichment.py`. Keep its query URLs, response
  hashes, truncation counts and NCBI policy provenance; never add runtime PubChem HTTP
  calls or imply that the snapshot is the complete PubChem Compound collection.
